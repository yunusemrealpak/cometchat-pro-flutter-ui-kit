...
class CometChatDataItem<T> extends StatelessWidget {
  ...
  Widget getAvatar(CometChatTheme _theme) {
    if (inputData.thumbnail) {
      ...
      return CometChatAvatar(
        ...
        nameTextStyle: avatarConfiguration.nameTextStyle ?? TextStyle(color: _theme.palette.getBackground(), fontSize: _theme.typography.name.fontSize, fontWeight: _theme.typography.name.fontWeight, fontFamily: _theme.typography.name.fontFamily),
  ...
  Widget getStatus(CometChatTheme _theme) {
    if (inputData.status) {
      ...
      Icon? icon;
      if (user != null) {
        backgroundColor = user!.status != null && user!.status == UserStatusConstants.online ? _theme.palette.getSuccess() : null;
      } else if (group != null) {
        if (group!.type == GroupTypeConstants.password) {
          ...
          icon = const Icon(Icons.privacy_tip_rounded, color: Colors.white, size: 7);
        }
      } else if (groupMember != null) {
        backgroundColor = groupMember!.status != null && groupMember!.status == UserStatusConstants.online ? _theme.palette.getSuccess() : null;
      }
  ...
  Widget? getTitle(CometChatTheme _theme) {
    if (inputData.title) {
      ...
      return Text(
        _title,
        style: style.titleStyle ?? TextStyle(color: _theme.palette.getAccent(), fontWeight: _theme.typography.name.fontWeight, fontSize: _theme.typography.name.fontSize, fontFamily: _theme.typography.name.fontFamily),
      );
    } else {
      return const SizedBox();
    }
    ...
    if (inputData.subtitle != null) {
      ...
      if (_subtitle != null) {
        return Text(
          ...
          style: style.subtitleStyle ?? TextStyle(color: _theme.palette.getAccent600(), fontSize: _theme.typography.subtitle1.fontSize, fontWeight: _theme.typography.subtitle1.fontWeight, fontFamily: _theme.typography.subtitle1.fontFamily),
        );
      } else {
        return null;
      }
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: style.background ?? _theme.palette.getBackground(), gradient: style.gradient, border: style.border, borderRadius: BorderRadius.circular(style.cornerRadius ?? 0)),
      child: ListTile(
...
class DataItemStyle {
  ...
  const DataItemStyle({this.height, this.width, this.background, this.border, this.cornerRadius, this.titleStyle, this.subtitleStyle, this.gradient});
